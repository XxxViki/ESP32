...
static size_t rmt_encode_led_strip(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size, rmt_encode_state_t *ret_state)
{
    ...
    */
    ...
    case 0: // send RGB data
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, primary_data, data_size, &session_state);  //encode函数是字节编码器的回调函数，将用户数据编码成RMT符号
...
esp_err_t ws2812_init(gpio_num_t gpio,int maxled,ws2812_strip_handle_t* handle)
{
    ...
    rmt_tx_channel_config_t tx_chan_config = {
        ...
        .mem_block_symbols = 64,                //内存块大小，即 64 * 4 = 256 字节,RMT符号的个数
